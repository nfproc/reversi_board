...
void reversi_accel_soft(unsigned char *data_in, float *rho_out, float *theta_out, int height, int width);
#ifdef NO_HLS
typedef unsigned char inimg_t;
#define reversi_accel reversi_accel_soft
#else
#include <ap_int.h>
typedef ap_uint<PTRWIDTH> inimg_t;
void reversi_accel(inimg_t *data_in, float *rho_out, float *theta_out, int height, int width);
#endif
...
void my_detect_board(cv::Mat &image, float *rho_out, float *theta_out, int is_hard) {
    ...
    cv::GaussianBlur(image, tmp, cv::Size(3, 3), 1);
    ...
    // Hough transform
    if (! is_hard) {
        reversi_accel_soft(edge.data, rho_out, theta_out, edge.rows, edge.cols);
    } else {
        reversi_accel((inimg_t *) edge.data, rho_out, theta_out, edge.rows, edge.cols);
    }
}

// software version of accelerator, for comparison
void reversi_accel_soft(unsigned char *data_in, float *rho_out, float *theta_out, int height, int width) {
    cv::Mat img_in = cv::Mat(height, width, CV_8UC1, data_in);
    std::vector<cv::Vec2f> lines;
    xiHoughLinesstandard(img_in, lines, 3.0, 3.0 * CV_PI / 180, 500, 32, 180, 0);
