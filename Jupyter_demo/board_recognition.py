...
class RealBoardRecognizer(BaseRecognizer):
    ...
    def _detectConvexHull(self, image):
        ...
        # 領域の輪郭を抽出
        contours, hierarchy = cv2.findContours(greenWhite, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2:]
        ...
        # 凸包を再度取得する
        contours, hierarchy = cv2.findContours(green, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2:]
...
class ScreenshotRecognizer(RealBoardRecognizer):
    ...
    def _detectConvexHull(self, image):
        ...
        # 最大領域の輪郭を取得する
        contours, _ = cv2.findContours(maxImg, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2:]
...
class PrintedBoardRecognizer(RealBoardRecognizer):
    ...
    def _detectConvexHull(self, image):
        ...
        # 領域の輪郭を抽出
        contours, hierarchy = cv2.findContours(blackImage, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2:]
        for i, c in enumerate(contours):
            # 4点未満の場合はスキップ
            if len(contours[i]) < 4:
                continue
